...
public interface SchoolDepartmentRepository extends CrudRepository<SchoolDepartment, Long>, JpaSpecificationExecutor<SchoolDepartment> {
    ...
    @Query("select new ee.hitsa.ois.web.dto.SchoolDepartmentDto(sd.id, sd.code, sd.nameEt, sd.nameEn, sd.validFrom, sd.validThru, sd.parentSchoolDepartment.id) from SchoolDepartment sd where sd.school.id=?1")
