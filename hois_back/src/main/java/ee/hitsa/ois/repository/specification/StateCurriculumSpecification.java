...
public class StateCurriculumSpecification implements Specification<StateCurriculum> {
	...
	public Predicate toPredicate(Root<StateCurriculum> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
		...
		if(searchCommand.getValidFromMillis() != null) {
			LocalDate validFrom = millisToLocalDate(searchCommand.getValidFromMillis());
			filters.add(cb.greaterThanOrEqualTo(root.get("validFrom"), validFrom));
		}
		if(searchCommand.getValidThruMillis() != null) {
			LocalDate validThru = millisToLocalDate(searchCommand.getValidThruMillis());
	...
	private static LocalDate millisToLocalDate(Long millis) {
		return Instant.ofEpochMilli(millis.longValue()).atZone(ZoneId.systemDefault()).toLocalDate();
	}
}
