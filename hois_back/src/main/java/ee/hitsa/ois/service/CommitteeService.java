...
public class CommitteeService {
    ...
    private final String COMMITTEE_SELECT = "distinct c.id, "
    ...
    private final String COMMITTEE_FROM = " from committee c "
    ...
    private final String MEMBER_SELECT = " distinct "
            + "(case when cm.member_name is not null "
            + "then cm.member_name "
            + "else p.firstname || ' ' || p.lastname "
            + "end), "
            + "cm.teacher_id ";
    private final String MEMBER_FROM = " from committee_member cm "
    ...
    public Page<CommitteeSearchDto> search(Long schoolId, CommitteeSearchCommand criteria, Pageable pageable) {
        ...
        qb.groupBy(" c.id ");
        return JpaQueryUtil.pagingResult(qb, COMMITTEE_SELECT, em, pageable)
                .map(this::searchResultToDto);
    }
    
    private CommitteeSearchDto searchResultToDto(Object row) {
        ...
        if(members != null) {
            dto.setMembers(new HashSet<>(Arrays.asList(members.split(", "))));
    ...
    public Committee create(Long schoolId, CommitteeDto dto) {
        ...
        committee.setSchool(schoolRepository.getOne(schoolId));
    ...
    public CommitteeMember updateMember(CommitteeMemberDto dto, CommitteeMember member) {
        ...
        if(Boolean.FALSE.equals(dto.getIsExternal())) {
            member.setTeacher(teacherRepository.getOne(dto.getTeacher()));
    ...
    public Set<AutocompleteResult> getMembers(Long schoolId) {
        ...
        return StreamUtil.toMappedSet(r -> {
            ...
            Long id = resultAsLong(r, 1);
            return new AutocompleteResult(id, name, name);
        }, members);
    }
    
    public void delete(Committee committee) {
        EntityUtil.deleteEntity(committeeRepository, committee);
    }
}
