...
public class ReportService {
    ...
    public Page<StudentStatisticsDto> studentStatistics(Long schoolId, StudentStatisticsCommand criteria, Pageable pageable) {
        ...
        if(!result.getContent().isEmpty()) {
            ...
            loadStudentStatistics(cs, groupingField, criteria);
            if(MainClassCode.FINALLIKAS.name().equals(criteria.getResult())) {
                // Fin source is grouped twice: fin source and fin source explanation
                loadStudentStatistics(cs, "sh.fin_specific_code", criteria);
            }
        }
        return result;
    }
    ...
    public Page<StudentStatisticsDto> studentStatisticsByPeriod(Long schoolId, StudentStatisticsByPeriodCommand criteria, Pageable pageable) {
        ...
        if(!result.getContent().isEmpty()) {
            ...
            qb.optionalCriteria("d.valid_from <= :validThru", "validThru", criteria.getThru());
    ...
    public Page<TeacherLoadDto> teacherLoad(Long schoolId, TeacherLoadCommand criteria, Pageable pageable) {
        ...
        if(!result.getContent().isEmpty()) {
            ...
            List<?> subjects = qb.select("s.name_et, s.name_en, s.code, 0, sspt.teacher_id, ssp.study_period_id", em).getResultList();
            subjectRecords = subjects.stream().collect(Collectors.groupingBy(r -> resultAsLong(r, 4), Collectors.groupingBy(r -> resultAsLong(r, 5))));
        } else {
    ...
    private Page<StudentStatisticsDto> loadCurriculums(Long schoolId, List<Long> curriculumIds, Pageable pageable) {
        ...
        qb.optionalCriteria("c.id in (:curriculum)", "curriculum", curriculumIds);

        return JpaQueryUtil.pagingResult(qb, "c.id, c.name_et, c.name_en", em, pageable).map(r -> new StudentStatisticsDto(r));
    }

    private void loadStudentStatistics(Map<Long, StudentStatisticsDto> curriculums, String groupingField, StudentStatisticsCommand criteria) {
    ...
    private static void loadStatisticCounts(Map<Long, StudentStatisticsDto> curriculums, List<?> data) {
        for(Object r : data) {
            ...
            if(group != null) {
                curriculums.get(curriculumId).getResult().put(group, resultAsLong(r, 2));
            }
        }
    }
}
