    ...
    private static String convertHtmlToXslFo(String html, CertificateType type) {
        ...
        if (isContentEditable(type)) {
            ...
            result = result
                    .replaceAll("[\n|\r]", "")
                    .replaceAll("<br>", "\r")
                    .replaceAll("<p>", "<fo:block>").replaceAll("</p>", "</fo:block>")
        ...
        if (CertificateType.TOEND_LIIK_SOOR.equals(type)) {
            result = result
                    .replaceAll("<table class=\"widerTable\">", "<fo:block><fo:leader/></fo:block>"
                            ...
                            + "<fo:table-column/>").replaceAll("</table>", "</fo:table><fo:block><fo:leader/></fo:block><fo:block><fo:leader/></fo:block>")
                    .replaceAll("<thead>", "<fo:table-header text-align=\"center\" font-weight=\"bold\">").replaceAll("</thead>", "</fo:table-header>")
