...
@Component
public class SubjectProgramValidation {
    ...
    public boolean canView(HoisUserDetails user, SubjectProgram program) {
        ...
        JpaNativeQueryBuilder qb = new JpaNativeQueryBuilder("from subject_program sp " +
                ...
                "join subject_study_period_teacher sspt on sspt.id = spt.subject_study_period_teacher_id " +
                "join subject_study_period ssp on ssp.id = sspt.subject_study_period_id " +
                "left join curriculum_version_hmodule_subject cvhms on cvhms.subject_id = ssp.subject_id " +
                ...
                "left join curriculum c on c.id = cv.curriculum_id ").limit(1);
    ...
    private School subjectProgramSchool(SubjectProgram program) {
        List<School> schools = em.createQuery("select sp.subjectStudyPeriod.subject.school " +
                "from SubjectProgram sp " +
                "where sp.id = :programId", School.class)
    ...
    public boolean canEdit(HoisUserDetails user, SubjectProgram program) {
        ...
        if (user.isTeacher() && program.getTeachers().stream()
                .map(SubjectProgramTeacher::getSubjectStudyPeriodTeacher)
                .noneMatch(sspt -> EntityUtil.getId(sspt.getTeacher()).equals(user.getTeacherId()))) {
            return false;
        }
        if (user.isSchoolAdmin() && !user.getSchoolId().equals(
                EntityUtil.getId(program.getSubjectStudyPeriod().getSubject().getSchool()))) {
            return false;
        }
        return true;
    }
    ...
    public boolean canComplete(HoisUserDetails user, SubjectProgram program) {
        ...
        if ((user.isTeacher() && program.getTeachers().stream()
                .map(SubjectProgramTeacher::getSubjectStudyPeriodTeacher)
                .noneMatch(sspt -> EntityUtil.getId(sspt.getTeacher()).equals(user.getTeacherId())))
            || (user.isSchoolAdmin() && !user.getSchoolId().equals(
                EntityUtil.getId(program.getSubjectStudyPeriod().getSubject().getSchool())))) {
            return false;
        }
