...
public class BaseModuleController {
    ...
    @PutMapping("/{id:\\d+}")
    public BaseModuleDto save(HoisUserDetails user,
            @NotNull @Valid @RequestBody BaseModuleForm baseModuleForm, @WithEntity BaseModule baseModule) {
    ...
    public List<OccupiedAutocompleteResult> getTeachers(HoisUserDetails user, TeacherAutocompleteCommand lookup) {
        if (!(UserUtil.canViewBaseModule(user) || UserUtil.hasPermission(user, Permission.OIGUS_M, PermissionObject.TEEMAOIGUS_OPPEKAVA))) {
    ...
    public BaseModuleDto generateBaseModule(HoisUserDetails user, @WithEntity("cId") CurriculumModule cModule, @WithEntity("oId") CurriculumVersionOccupationModule oModule) {
