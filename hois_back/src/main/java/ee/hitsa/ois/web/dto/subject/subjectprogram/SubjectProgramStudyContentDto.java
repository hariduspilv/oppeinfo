...
public class SubjectProgramStudyContentDto extends VersionedCommand {
    ...
    public static SubjectProgramStudyContentDto of(SubjectProgramStudyContent content) {
        SubjectProgramStudyContentDto dto = EntityUtil.bindToDto(content, new SubjectProgramStudyContentDto());
        dto.setSubjectProgramId(EntityUtil.getId(content.getSubjectProgram()));
        dto.setTeachers(content.getTeachers().stream().map(t -> EntityUtil.getId(t.getTeacher().getSubjectStudyPeriodTeacher())).collect(Collectors.toList()));
        return dto;
    }
