...
public class CurriculumModuleDto extends VersionedCommand {
    ...
    public static CurriculumModuleDto of(CurriculumModule module) {
        ...
        if(module.getOutcomes() != null) {
            Set<CurriculumModuleOutcomeDto> outcomes = module.getOutcomes().stream().
                    map(a -> CurriculumModuleOutcomeDto.of(a)).collect(Collectors.toSet());
            dto.setOutcomes(outcomes);
        }
        if(module.getOccupations() != null) {
            Set<String> occupations = module.getOccupations().stream().
                    map(o -> EntityUtil.getNullableCode(o.getOccupation())).collect(Collectors.toSet());
            dto.setOccupations(occupations);
        }
        if(module.getCompetences() != null) {
            Set<String> competences = module.getCompetences().stream().
                    map(c -> EntityUtil.getNullableCode(c.getCompetence())).collect(Collectors.toSet());
